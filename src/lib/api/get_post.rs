...
use crate::api::data_structs::{Attachment, Post, User};
...
fn user_by_uuid(users: &[PostsResponseUser], uuid: &str) -> Option<PostsResponseUser> {
        ...
        match self.get_from_api(&path).await {
            Ok(response) => match response.json::<PostsResponse>().await {
                Ok(response_struct) => {
                    ...
                    let attachments = response_struct
                        .attachments
                        .iter()
                        .map(|att| Attachment {
                            id: att.id.clone(),
                    ...
                    match author {
                        ...
                        None => Err(FreefeedApiError::UnknownParseError),
                    }
                }
                Err(_e) => Err(FreefeedApiError::UnknownParseError),
            },
            Err(e) => Err(e),
        }
    }
